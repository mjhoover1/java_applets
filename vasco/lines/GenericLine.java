...
public abstract class GenericLine extends LineStructure implements MaxDecompIface {
	...
	boolean res = PtOnBoundary(q.P1.x, q.P1.y, s) || PtOnBoundary(q.P2.x, q.P2.y, s) || CSquare(q.P1.x, q.P1.y, q.P2.x, q.P2.y, s);
	return res;
    }
	...
	for (int i = 0; i < 4; i++) {
	    ...
	    Q.NODETYPE = BLACK;
	    S = new QSquare();
	    S.CENTER = new DPoint(P.SQUARE.CENTER.x + XF[i] * P.SQUARE.LEN, 
				  P.SQUARE.CENTER.y + YF[i] * P.SQUARE.LEN);
	...
	if (S.CENTER.x - S.LEN / 2 <= p.x && p.x <= S.CENTER.x + S.LEN / 2 &&
	    S.CENTER.y - S.LEN / 2 <= p.y && p.y <= S.CENTER.y + S.LEN / 2)
	    return true;
	else
	    return false;
    }
	...
	return (s.CENTER.x - s.LEN/2 <= x && x <= s.CENTER.x + s.LEN/2 && (y == s.CENTER.y - s.LEN/2 || y == s.CENTER.y + s.LEN/2)) ||
		(s.CENTER.y - s.LEN/2 <= y && y <= s.CENTER.y + s.LEN/2 && (x == s.CENTER.x - s.LEN/2 || x == s.CENTER.x + s.LEN/2));
    }
	...
	if (code1 == 0 || code2 == 0) // one vertex inside, line intersects
	    return true;
	if ((code1 & code2) != 0) // vertices outside in the same direction, no intersection
	    return false;
    ...
    //   void lineInOut(DLine l, DRectangle searchRect, int mode, SearchVector sv, Vector searchVector) {
    //     boolean isBlue = false;
    //     if ((mode & SEARCHMODE_ISCONTAINED) != 0) // both points inside
    //       isBlue = isBlue || (searchRect.contains(l.p1) && searchRect.contains(l.p2)); 
    //     if ((mode & SEARCHMODE_CROSSES) != 0) // line intersects but no endpoints are inside
	...
	DLine[] Query(QueryObject qu, SearchVector ret, double dist, int nrelems) {
		...
		if (e instanceof QueueLeaf) {
		    ...
		    ret.addElement(new NNElement(new NNDrawable(((QueueLeaf)e).line, counter++), 
						 e.key, q.makeVector()));
		} else {
		    ret.addElement(new NNElement(new YellowBlock(((QueueINode)e).box, false), 
						 e.key, q.makeVector()));
		}
		...
		if (e instanceof QueueINode) {
			...
			for (QEdgeList l = qi.r.DICTIONARY; l != null; l = l.NEXT) {
				...
				q.Enqueue(new QueueLeaf(qu.distance(toLine(l.DATA)), 
							l.DATA.P1.x, l.DATA.P1.y,
