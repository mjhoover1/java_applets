...
public class LineCanvas extends GenericCanvas implements FileIface, 
							  ItemListener {
  ...
  public LineCanvas(DRectangle can, DrawingTarget dt, DrawingTarget over, JPanel animp, TopInterface ti) {
    ...
    opFeature[2] = new OpFeature("Move vertex", OPFEATURE_MOVEVERTEX, 
				 ...
				 "Move existing vertex (use Control to snap)", "", "", 
				 InputEvent.BUTTON1_MASK);

    opFeature[3] = new OpFeature("Move collection", OPFEATURE_MOVECOLLECTION, 
				 "Click and drag to move all lines.", 
				 "Move all lines", "", "", 
				 InputEvent.BUTTON1_MASK);
    opFeature[4] = new OpFeature("Rotate collection", OPFEATURE_ROTATECOLLECTION, 
				 "Click and drag to rotate all lines.", 
				 "Rotate all lines", "", "", 
				 InputEvent.BUTTON1_MASK);
  ...
  protected void nearest(QueryObject p, double dist, DrawingTarget[] off) {
	...
	return QueryObject.QO_RECTANGLE | QueryObject.QO_POLYGON | 
	    QueryObject.QO_LINE | QueryObject.QO_PATH  | QueryObject.QO_SECTOR;
    }
  ...
  public void mousePressed(MouseEvent me) {
	...
	if (lastDelete != null) {
	    ...
	    pstruct.Insert(dl);
	    pstruct.MessageEnd();
	    lastMove = new moveLines[((drw1 != null) ? drw1.length : 0) + 
				    ((drw2 != null) ? drw2.length : 0) + 1];
...
System.out.println("update fails");
	...
	for (int i = 0; i < lastMove.length; i++) {
	    ...
	    if (!wholeCanvas.contains(p1) ||
		!wholeCanvas.contains(p2)) {
		allvalid = false;
		break;
	    }
	}
	...
	for (int i = 0; i < lastMove.length; i++) {
	    ...
	    DPoint p1 = new DPoint(moveStart.x + (tp1.x * Math.cos(angleDiff) + tp1.y * Math.sin(angleDiff)), moveStart.y + (-tp1.x * Math.sin(angleDiff) + tp1.y * Math.cos(angleDiff)));
	    DPoint tp2 = new DPoint(lastMove[i].line.p2.x - moveStart.x, lastMove[i].line.p2.y - moveStart.y);
	    DPoint p2 = new DPoint(moveStart.x + (tp2.x * Math.cos(angleDiff) + tp2.y * Math.sin(angleDiff)), moveStart.y + (-tp2.x * Math.sin(angleDiff) + tp2.y * Math.cos(angleDiff)));
...
public void mouseReleased(MouseEvent me) {
	...
	if (!(last.x == p.x && last.y == p.y) && pstruct.Insert(l))
	    ...
	    String errMesg = (last.x == p.x && last.y == p.y) ? 
			   "Isolated vertex rejected" : "Subdivision too deep, line rejected";
