...
public class QueryObject implements Drawable {
	...
	private void fillPointset(Color front, Color back, DrawingTarget dt, double dist) {
		...
		for (int i = 0; i < qov.Size(); i++) {
			...
			for (int j = 0; j < p.length - (close ? 0 : 1); j++) {
				...
				esize = Math.sqrt((e.p1.x - e.p2.x) * (e.p1.x - e.p2.x) + (e.p1.y - e.p2.y) * (e.p1.y - e.p2.y));
	...
	public boolean overlaps(Drawable r) {
		...
		if (qo instanceof DPoint)
			...
			if (r instanceof ArealObject) {
				...
				return ((rec.contains(l.p1) && !rec.contains(l.p2)) || (!rec.contains(l.p1) && rec.contains(l.p2)));
			} else
				return false;
	...
	public boolean contains(Drawable r) {
		...
		for (int i = 0; i < qov.Size(); i++) {
			if (qov.drawableGet(i) instanceof DPoint || qov.drawableGet(i) instanceof DLine
					|| qov.drawableGet(i) instanceof DPath)
				continue;
			if (qov.drawableGet(i) instanceof DRectangle || qov.drawableGet(i) instanceof DPolygon) {
				...
				if (r instanceof DPoint && rec.contains((DPoint) r) || r instanceof DLine && rec.contains((DLine) r)
	...
	public boolean crosses(Drawable r) {
		...
		if (qo instanceof DPoint) {
			...
			if (r instanceof DPoint)
				...
				return (rec.intersects(dl) && !rec.contains(dl.p1) && !rec.contains(dl.p2));
			}
			return false;
		} else if (qo instanceof DRectangle) {
			DRectangle rec = (DRectangle) qo;
			if (r instanceof DPoint)
				...
				return rec.intersects(rec2)
						&& !(rec.contains(rec2.SWcorner()) || rec.contains(rec2.SEcorner())
								|| rec.contains(rec2.NWcorner()) || rec.contains(rec2.NEcorner()))
						&& !(rec2.contains(rec.SWcorner()) || rec2.contains(rec.SEcorner())
								|| rec2.contains(rec.NWcorner()) || rec2.contains(rec.NEcorner()));
			} else
				return false;
		} else if (qo instanceof DPolygon) {
			DPolygon rec = (DPolygon) qo;
			if (r instanceof DPoint)
				...
				return rec.intersects(rec2) && !(rec.contains(rec2.SWcorner()) || rec.contains(rec2.SEcorner())
						|| rec.contains(rec2.NWcorner()) || rec.contains(rec2.NEcorner())) && !inside;
			} else
				return false;
		} else if (qo instanceof DPath) {
			DPath dl = (DPath) qo;
