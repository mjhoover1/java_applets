...
public class ColorHelp extends JDialog implements ActionListener {
	...
	String[] deleteHelp = { "Objects in the tree.", "Object that would be deleted." };

	Color[] deleteColors = { Color.red, Color.orange };

	String[] moveHelp = { "Objects in the tree.", "Object that would be or is being moved." };
	Color[] moveColors = { Color.red, Color.orange };

	String[] moveVertexHelp = { "Objects in the tree.", "Vertex that would be or is being moved." };

	String[] moveEdgeHelp = { "Objects in the tree.", "Edge that would be or is being moved." };

	String[] windowHelp = { "Blocks that remain to be processed (i.e., they partially overlap the query range).",
			...
			"The next item to be processed (could be a block or an object).", "The query range." };
	Color[] windowColors = { Color.cyan, Color.green, Color.red, Color.blue, Color.magenta, Color.gray, Color.white,
			Color.yellow, Color.orange };

	String[] nearestHelp = { "Blocks in the priority queue.", "Objects in the priority queue.",
			...
			"The query range." };
	Color[] nearestColors = { Color.cyan, Color.green, Color.red, Color.blue, Color.gray, Color.yellow, Color.white,
			Color.orange };

	String[] bintreeHelp = { "Bintree structure.", "Center of quadrant for which the bintrees are being displayed." };
	...
	helpStruct[] help = { new helpStruct("Insert", "Insert legend", insertHelp, insertColors),
	...
	public ColorHelp(String opType, int obj_nr) {
		...
		for (int i = 0; i < help[legIndex].opColor.length; i++) {
			...
			JPanel p = createPanel(
					((help[legIndex].opColor[i] == Color.orange && opType.equals("Insert") && obj_nr == LINE_APPLET)
							? lineInsert
							: help[legIndex].opText[i]),
