...
public class DrawingCanvas extends Canvas implements DrawingTarget {
  ...
  void moveX(double p) {
    ...
    target = new Rectangle(Math.round((float)(viewPort.x + percent * (viewPort.width - target.width))), 
    			   target.y, target.width, target.height);
    //    target = new Rectangle(Math.round((float)(orig.x + percent * (orig.width - target.width))), 
    //		   target.y, target.width, target.height);
  }
  ...
  void moveY(double p) {
    ...
    target = new Rectangle(target.x, 
			   Math.round((float)(viewPort.y + percent * (viewPort.height - target.height))), 
  ...
  private Point transVector(double dx, double dy) {
    return new Point((int)(target.width * dx / orig.width), 
		     (int)(target.height * dy / orig.height));
  }
  ...
  public void directThickRect(Color c, double xx, double yy, 
	                    double ww, double hh, int thk) {
    Graphics cur = getGraphics();
