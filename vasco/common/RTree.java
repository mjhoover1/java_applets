...
public class RTree extends SpatialStructure implements ItemListener {
	...
	class RTreeNode {
		...
		RTreeNode quadratic() {
			...
			while (forward <= backward) {
				...
				loop: for (int i = 0; i < rarray.length; i++) {
		...
		RTreeNode ang() {
			...
			for (int i = 0; i < rarray.length; i++) {
				if (Math.abs(rarray[i].boundingBox.x - boundingBox.x) < Math
						.abs(rarray[i].boundingBox.x + rarray[i].boundingBox.width - boundingBox.x - boundingBox.width))
					left.addElement(new Integer(i));
				else
					right.addElement(new Integer(i));
				if (Math.abs(rarray[i].boundingBox.y - boundingBox.y) < Math.abs(
						rarray[i].boundingBox.y + rarray[i].boundingBox.height - boundingBox.y - boundingBox.height))
		...
		void axisSplit(RTreeNode n1, RTreeNode n2) {
			...
			for (int i = 0; i < rarray.length; i++)
				for (int j = 0; j < i; j++) {
					...
					if (rarray[i].boundingBox.y < rarray[j].boundingBox.y)
						swap(minY, i, j);
					if (rarray[i].boundingBox.x + rarray[i].boundingBox.width < rarray[j].boundingBox.x
							+ rarray[j].boundingBox.width)
						swap(maxX, i, j);
					if (rarray[i].boundingBox.y + rarray[i].boundingBox.height < rarray[j].boundingBox.y
							+ rarray[j].boundingBox.height)
						swap(maxY, i, j);
				}
			...
			for (int splitpoint = minNodeLength - 1; splitpoint <= rarray.length - 1 - minNodeLength; ++splitpoint) {
			...
			for (int minmax = 0; minmax < 2; ++minmax) {
				...
				for (int splitpoint = minNodeLength - 1; splitpoint < rarray.length - minNodeLength; ++splitpoint) {
	...
	final static String[] structs = { "Exhaustive", "Quadratic", "Linear", "R* tree", "Ang/Tan", "Hilbert packed",
			"Morton packed", "Hilbert nonpacked", "Morton nonpacked", "packed" };
	...
	public void drawGrid(DrawingTarget g, int gridLevel) {
		if (gridLevel < maxSpaceFillingCurveLevel) {
			if (splitMode.equals("Hilbert packed") || splitMode.equals("Hilbert nonpacked")
					|| splitMode.equals("packed")) {
				g.setColor(Color.black);
				for (int i = 0; i < hilbert[gridLevel].length - 1; i++) {
					g.drawLine(hilbert[gridLevel][i].x, hilbert[gridLevel][i].y, hilbert[gridLevel][i + 1].x,
							hilbert[gridLevel][i + 1].y);
				}
			}
			if (splitMode.equals("Morton packed") || splitMode.equals("Morton nonpacked")) {
				g.setColor(Color.black);
				for (int i = 0; i < morton[gridLevel].length - 1; i++) {
					g.drawLine(morton[gridLevel][i].x, morton[gridLevel][i].y, morton[gridLevel][i + 1].x,
							morton[gridLevel][i + 1].y);
				}
			}
		}
	}
	...
	int getHilbert(DRectangle r) {
		...
		return getHilbert((int) (DEFSIZE * ((r.x + r.width / 2) - wholeCanvas.x) / wholeCanvas.width),
				(int) (DEFSIZE * ((r.y + r.height / 2) - wholeCanvas.y) / wholeCanvas.height), DEFSIZE);
	}
	...
	int getHilbert(double ii, double jj, double size) {
		...
		int quadTable[][][] = { { { 0, 1 }, { 3, 2 } }, { { 1, 2 }, { 0, 3 } }, { { 2, 3 }, { 1, 0 } },
				{ { 3, 0 }, { 2, 1 } } };
		...
		for (int k = 0; k < maxSpaceFillingCurveLevel; k++) {
			...
			for (int i = 0; i < (int) (canvasWidth / add); i++) {
				for (int j = 0; j < (int) (canvasWidth / add); j++) {
					...
					hilbert[k][getHilbert(i, j, (int) Math.pow(2, k))] = new DPoint(wholeCanvas.x + add / 2 + i * add,
							wholeCanvas.y + add / 2 + j * add);
				}
			}
		}
		...
		if (coverage.length > 0) {
			...
			for (int i = 0; i < coverage.length; i++) {
				...
				if (i != coverage.length - 1)
					coverage[i] = Math.max(1.0, sp.getArea() / basearea);
				overlap[i] = Math.max(1.0, areaSum(i) / basearea);
			}
		}
	}
	...
	void searchLoc(RTreeNode n, QueryObject searchRect, int mode, SearchVector v, Vector searchVector) {
		...
		v.addElement(new SVElement(
				new YellowBlock(n.boundingBox, !(n instanceof RTreeLeaf) && n.rarray[0] instanceof RTreeLeaf),
				searchVector));
	...
	/*
	 * void chooseReinsert(RTreeNode n) { for (int i = 0; i < n.occup; i++) { for
	 * (int j = 0; j < i; j++) { double checkDistI = RstarDistance(n.boundingBox,
	 * n.rarray[i].boundingBox); double checkDistJ = RstarDistance(n.boundingBox,
	 * n.rarray[j].boundingBox); if (checkDistI < checkDistJ) { RTreeNode rt =
	 * n.rarray[i]; n.rarray[i] = n.rarray[j]; n.rarray[j] = rt; } } } }
	 */
	...
	RTreeNode[] insertRstarLoc(Drawable r, RTreeNode n, RTreeNode origin) {
		...
		if (n.rarray[0] instanceof RTreeLeaf) {
			...
			if (n.occup < maxNodeLength) {
				...
				n.occup = split;
				for (Drawable element : box) {
					RTreeNode[] ar = insertRstarLoc(element, root, n);
					if (ar != null) {
						root = new RTreeNode(null, minNodeLength, maxNodeLength);
						root.insert(ar[0]);
						root.insert(ar[1]);
	...
	void insert(Drawable r) {
		if (root == null) {
			root = new RTreeLeaf(r, minNodeLength, maxNodeLength);
			root.btreeKey = splitMode.equals("Hilbert nonpacked") ? getHilbert(r.getBB()) : getMorton(r.getBB());
		} else {
			if (root instanceof RTreeLeaf) {
				RTreeNode oldr = root;
				root = new RTreeNode(r.getBB().union(oldr.boundingBox), minNodeLength, maxNodeLength);
				root.rarray[root.occup++] = oldr;
				root.rarray[root.occup++] = new RTreeLeaf(r, minNodeLength, maxNodeLength);
				root.rarray[1].btreeKey = splitMode.equals("Hilbert nonpacked") ? getHilbert(r.getBB())
						: getMorton(r.getBB());
	...
	RTreeNode insertLoc(Drawable r, RTreeNode n) {
		...
		if (n.rarray[0] instanceof RTreeLeaf) { // second level from bottom
			...
			rt.btreeKey = splitMode.equals("Hilbert nonpacked") ? getHilbert(r.getBB()) : getMorton(r.getBB());
		...
		if (splitMode.equals("Hilbert nonpacked") || splitMode.equals("Morton nonpacked")) {
			...
			for (i = 0; i < n.occup - 1; i++)
				if (n.rarray[i].btreeKey > (splitMode.equals("Hilbert nonpacked") ? getHilbert(r.getBB())
						: getMorton(r.getBB())))
					break;
	...
	/*
	 * void sortHilbert(Vector v, int startNode) { for (int i = startNode; i <
	 * v.size() - 1; i++) for (int j = startNode; j < v.size() - 1; j++) {
	 * DRectangle r1 = ((RTreeNode)v.elementAt(j)).boundingBox; DRectangle r2 =
	 * ((RTreeNode)v.elementAt(j+1)).boundingBox; if (getHilbert(r1) >
	 * getHilbert(r2)) { RTreeNode b = (RTreeNode)v.elementAt(j);
	 * v.setElementAt(v.elementAt(j+1), j); v.setElementAt(b, j+1); } } }
	 */
	// insert Hilbert --------------------------------------------
	...
	Vector buildRossopulos(Vector v) {
		...
		for (int i = 0; i < v.size();) {
			...
			/*
			 * for (int k = i + 1; k < v.size() - 1; k++) for (int j = i + 1; j < v.size() -
			 * 1; j++) if
			 * (((RTreeNode)v.elementAt(j)).boundingBox.distance(((RTreeNode)v.elementAt(i))
			 * .boundingBox) >
			 * ((RTreeNode)v.elementAt(j+1)).boundingBox.distance(((RTreeNode)v.elementAt(i)
			 * ).boundingBox)) { RTreeNode b = (RTreeNode)v.elementAt(j);
			 * v.setElementAt(v.elementAt(j+1), j); v.setElementAt(b, j+1); }
			 */
	...
	void insertMorton() {
		...
		/*
		 * for (int i = 0; i < v.size() - 1; i++) for (int j = 0; j < v.size() - 1; j++)
		 * { Drawable r1 = (Drawable)v.elementAt(j); Drawable r2 =
		 * (Drawable)v.elementAt(j+1); if (getMorton(r1.getBB()) >
		 * getMorton(r2.getBB())) { Drawable b = (Drawable)v.elementAt(j);
		 * v.setElementAt(v.elementAt(j+1), j); v.setElementAt(b, j+1); } }
		 */
		Vector newV = new Vector();
		for (int i = 0; i < v.size(); i++) {
	...
	void drawLevel(RTreeNode r, DrawingTarget g, int lev, int target, int counter[], Rectangle view) {
		/*
		 * if (!g.visible(r.boundingBox)) - messes up the numbers return;
		 */
		// System.out.println("-------");
		if (r == null)
			return;
		...
		class RTreeQueue {
			...
			void Enqueue(RTreeQueueElement qe) {
				...
				for (int i = v.size() - 1; i > 0; i--) {
					...
					if (q1.key1 > q2.key1 || (q1.key1 == q2.key1
		...
		private Drawable[] Query(QueryObject qu, SearchVector ret, double dist, int nrelems) {
			...
			while (!q.isEmpty()) {
				...
				if (element instanceof RTreeQLeaf) { // spatial object
					...
					ret.addElement(new NNElement(new NNDrawable(rl.geom, counter++), rl.key1, q.makeVector()));
				} else {
	...
	class verticalList {
		...
		double evaluateLine() {
			...
			/*
			 * for (int i = 0; i < vert.size() - 1; i++) // sort vertical list by 'y'
			 * coordinates for (int j = 0; j < vert.size() - 1; j++) { if
			 * (((sweepVert)vert.elementAt(j)).ycoord >
			 * ((sweepVert)vert.elementAt(j+1)).ycoord) { Object s = vert.elementAt(j);
			 * vert.setElementAt(vert.elementAt(j+1), j); vert.setElementAt(s, j+1); } }
			 */
	...
	class sweepPlane {
		...
		sweepPlane(Vector v) {
			...
			for (int i = 0; i < v.size(); i++) {
				...
				se[2 * i + 1] = new sweepEntry(false, ar[i].x + ar[i].width, ar[i].y, ar[i].y + ar[i].height,
						se[2 * i]);
			}

			QSortAlgorithm.sort(se);
			/*
			 * for (int i = 0; i < se.length - 1; i++) for (int j = 0; j < se.length - 1;
			 * j++) { if (se[j].xcoord > se[j+1].xcoord) { sweepEntry s = se[j]; se[j] =
			 * se[j+1]; se[j+1] = s; } }
			 */
		}
		...
		double getArea() {
			...
			for (sweepEntry element : se) {
				height = vl.evaluateLine();
				totalArea += height * (element.xcoord - lastX);
				lastX = element.xcoord;
				if (element.first)
					vl.add(element);
				else
					vl.remove(element);
