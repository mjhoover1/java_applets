...
public class DLine implements Drawable {
  ...
  public DLine(DPoint pt1, DPoint pt2) {
    p1 = pt1; p2 = pt2;
    bbox = new DRectangle(Math.min(p1.x, p2.x), Math.min(p1.y, p2.y), 
  ...
  public boolean equals(DLine l) {
    return l != null && ((p1.equals(l.p1) && p2.equals(l.p2)) ||
...
//
   ...
   case Parallel: {
      ...
      if (classif1 == DPoint.InBetween) return p1.distance(l2);
      int classif2 = p2.classify (l2);
      if (classif2 == DPoint.InBetween) return p2.distance(l2);
      if (classif1 == classif2) {
	 return (classif1 == DPoint.BeforeFirst ?
		 distance(l2.p1) :
		 distance(l2.p2));
      }
      else {
...
//
{
   ...
   if (Math.abs(det) > CommonConstants.accuracy) {
      ...
      double s = (nm.x * mk.y - nm.y * mk.x) * detinv;
      double t = (lk.x * mk.y - lk.y * mk.x) * detinv;
      if (s < -CommonConstants.accuracy || s-1 > CommonConstants.accuracy || 
	  t < -CommonConstants.accuracy || t-1 > CommonConstants.accuracy) {
         return NotParallel;
      }
      else {
         return Intersecting;
	...
	double dx, dy, esize;
	dt.setColor(c);
	esize = Math.sqrt((p1.x - p2.x)*(p1.x - p2.x) + 
			  (p1.y - p2.y)*(p1.y - p2.y));
	dy = (p2.y - p1.y) / esize;
	dx = (p2.x - p1.x) / esize;

	dt.drawLine(p1.x + dy * dist, p1.y - dx * dist,
		    p2.x + dy * dist, p2.y - dx * dist);
	dt.drawLine(p1.x - dy * dist, p1.y + dx * dist,
		    p2.x - dy * dist, p2.y + dx * dist);
	...
	if (p1.y > p2.y || p1.y == p2.y && p1.x > p2.x) {
	    rangle = (p1.x - p2.x == 0 && p1.y - p2.y == 0) ? 
		0 : Math.atan2(p1.y - p2.y, p1.x - p2.x);
	    pt1 = p2; pt2 = p1;
	} else {
	    rangle = (p2.x - p1.x == 0 && p2.y - p1.y == 0) ? 
		0 : Math.atan2(p2.y - p1.y, p2.x - p1.x);
	    pt1 = p1; pt2 = p2;
	}
