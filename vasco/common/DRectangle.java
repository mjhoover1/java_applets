...
public class DRectangle implements Drawable, ArealObject {
  ...
  public boolean contains(DRectangle b) {
    return contains(new DPoint(b.x, b.y)) &&     
      contains(new DPoint(b.x + b.width, b.y)) && 
      contains(new DPoint(b.x, b.y + b.height)) && 
      contains(new DPoint(b.x + b.width, b.y + b.height));
  }
...
//
  ...
  private int ClipArea(double xx, double yy) {
    ...
    if (xx < this.x)
      ...
      if (xx > this.x + this.width)
        c = right;
    if (yy < this.y)
      c |= down;
    else
      if (yy > this.y + this.height)
	c |= up;
    return c;
  }
  ...
  public String toString() {
	...
	dt.drawArc(x + width - dist, y + height - dist, 
		   2*dist, 2*dist, 0, -90);
    }
