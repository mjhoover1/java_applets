...
public class DTriangle extends DPolygon implements java.lang.Comparable
{
	...
	{
		super(pts);
		if(pts.length!=3)throw new Exception("Not the right number of vertices for a triangle");
		resortPoints();
		computeCircumcircle();
	}
	...
	public ArrayList getDrawableLines(DPoint[] fixedPoints)
	{
		...
		for (int i = 0; i < 3; i++) {
			boolean dodraw=true;
			for(int j=0;j<fixedPoints.length;j++)
			{
				if(border[i].equals(fixedPoints[j]) || border[(i+1)%3].equals(fixedPoints[j]))dodraw=false;
			}
			if(dodraw)
	...
	public DTriangleEdge getEdge(Integer edge)
	{
		...
		{
			int e=edge.intValue();
			if(e==1)return new DTriangleEdge((Site)border[0],(Site)border[1]);
			else if(e==2)return new DTriangleEdge((Site)border[0],(Site)border[2]);
			else return new DTriangleEdge((Site)border[1],(Site)border[3]);
		}
	}
	...
	private void computeCircumcircle()
	{
		...
		double[] bvals={border[0].x,border[0].y,amag,border[1].x,border[1].y,bmag,border[2].x,border[2].y,cmag};
		double b=det(bvals);
	...
	{
		return pt[0]*(pt[5]*pt[7]-pt[4]*pt[8])-pt[1]*(pt[6]*pt[5]-pt[3]*pt[8])+pt[2]*(pt[6]*pt[4]-pt[3]*pt[7]);
	}
	...
	public Integer getSharedEdge(DTriangle tri)
	{
		for(int i=0;i<3;i++)
		{
			for(int j=i+1;j<3;j++)
			{
				...
				{
					if(i==0 && j==1)return new Integer(0);
					if(i==0 && j==2)return new Integer(1);
					if(i==1 && j==2)return new Integer(2);
				}
			}
		}
		return null;
	}
	public boolean containsEdgeInOrder(DPoint p1,DPoint p2)
	{//we're guaranteed that p1 comes before p2 in the array, so...
		for(int i=0;i<3;i++)
		{
			for(int j=i+1;j<3;j++)
			{
	...
	{
		//return "{("+border[0].x+","+border[0].y+"),("+border[1].x+","+border[1].y+"),("+border[2].x+","+border[2].y+")}";
	...
	{
		dt.drawOval(circumcircleCenter.x,circumcircleCenter.y,2*(int)circumcircleRadius,2*(int)circumcircleRadius);
	}
}