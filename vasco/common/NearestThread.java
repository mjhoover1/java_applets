...
public class NearestThread extends VascoThread {
    // --- use PCanvas for: 1) drawBackground,  2) drawGrid,  3) drawContents,  4) waitTime, 5) resetButtons
    double dist;
    int sr, sg, sb;
    double dr, dg, db;
    boolean blend;
    ...
    public NearestThread(SearchVector vv, QueryObject p, CanvasIface c, double dist, DrawingTarget[] g,
			 int sr, int sg, int sb, double dr, double dg, double db) {
	this(vv, p, c, dist, g);
	...
	for(int i = 0; i < off.length; i++) {
		...
		if (blend) {
		    //System.out.println((sr + counter * dr) + " " + (sg + counter * dg) + " " + (sb + counter * db));
		    off[i].setColor(new Color((int)(sr + counter * dr), 
					      (int)(sg + counter * dg), 
					      (int)(sb + counter * db)));
		    if (ne.isElement())
			counter++;
	...
	if (v != null && v.size() > 0) {
	    setProgress(0); 
	    do {
		/* rectangle (PointIn or YellowBlock) && vector of green points or cyan rects with green borders*/
		if (drawCurrentStep()) {
		    pc.setPause();
		    suspend();
		} else try {
