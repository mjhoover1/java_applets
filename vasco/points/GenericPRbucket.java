...
abstract class GenericPRbucket extends PointStructure implements MaxDecompIface, BucketIface {
  ...
  public void Delete(DPoint p) {
    ...
    ROOT = delete(prin.Query(new QueryObject(p)), ROOT, 
	...
	if (ROOT == null) 
	    return;
	ROOT = delete((DPoint)d, ROOT, wholeCanvas.x + wholeCanvas.width / 2, 
		      wholeCanvas.y + wholeCanvas.height / 2,
		      wholeCanvas.width, wholeCanvas.height);
    }
  ...
  private void search(PRbucketNode R, QueryObject qu, DRectangle block, int mode, SearchVector v) {
    ...
    DRectangle ne = new DRectangle(block.x + block.width / 2, block.y + block.height / 2, block.width / 2, block.height / 2);
  ...
  abstract PRbucketNode insert (DPoint p, PRbucketNode R, 
