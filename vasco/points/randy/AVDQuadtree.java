...
public class AVDQuadtree//uses a PR quadtree implementation...
{
	...
	public AVDQuadtree(ArrayList points,double epsilon,int t,int maxDecomp,DRectangle space,boolean showColor,boolean maxDecomposition) throws DepthExceededException
	{
		this.maxDecomposition=maxDecomposition;
		this.showColor=showColor;
		this.epsilon=epsilon;
		this.t=t;
		if(maxDecomposition)t=5;
	...
	public NearestNeighborQueue getNearestNeighborQueue(Drawable dst,Node root)
	{
		...
		while(!toReturn.isEmpty())
		{
			...
			/*next=(PQComparable)toReturn.remove();
			Node n=(Node)next.d;
			if(n instanceof GrayNode && !(n instanceof LightGrayNode))
			{
				GrayNode gn=(GrayNode)n;
				toReturn.add(new PQComparable(pt,gn.nw));
				toReturn.add(new PQComparable(pt,gn.ne));
				toReturn.add(new PQComparable(pt,gn.sw));
				toReturn.add(new PQComparable(pt,gn.se));
			}
			else if(n instanceof BlackNode)
			{
				BlackNode bn=(BlackNode)n;
				toReturn.add(new PQComparable(pt,bn.pt));
			}*/
		}
		return toReturn;
	}
	...
	public class PQComparable implements Comparable
	{
		...
		public double distance()
		{
			...
			if(dst instanceof DRectangle && d instanceof DPoint && ((DRectangle)dst).contains((DPoint)d))return 0;
			if(dst instanceof DPoint)return d.distance((DPoint)dst);//do actual IncrementalNearestNeighbor
			else return d.distance((DRectangle)dst);//do actual IncrementalNearestNeighbor
		}
		...
		public int compareTo(Object o)
		{
			if(o instanceof PQComparable)
			{
				...
				if(this.distance()<pqc.distance())return -1;
				else if(this.distance()>pqc.distance())return 1;
				else return 0;
			}
			else return -1;
		}
	...
	public abstract class Node extends DRectangle
	{
		...
		public Node(double minx,double miny,double maxx,double maxy,GrayNode parent) throws DepthExceededException
		{
			...
			else 
			{
				...
				if(depth>maxDepth)throw new DepthExceededException(this);
			}
			this.parent=parent;
		}
		...
		public void getNearestNeighbors() throws BadOperationException
		{
			...
			TreeSet arr=(TreeSet)o[0];
			double mindist=((Double)o[1]).doubleValue();
			...
			/*if(sameNearest==true)nearestneighbors=0;
			else nearestneighbors=-1;*/
			...
			try
			{
				if(nearestneighbors>t || (!(root instanceof WhiteNode) && nearestneighbors==-1) || maxDecomposition)
				{
			...
			{
				//nearestneighbors is already outside of the acceptable range, so it will be colored red anyway
		...
		{
			if(minx<=p.x && miny<=p.y && maxx>p.x && maxy>p.y)return true;
			else return false;
		}
		...
		public void drawContents(DrawingTarget dt,Rectangle r)
		{
			...
			Color c;
			if(showColor && this.approximateNearestSite!=null)c=this.approximateNearestSite.getColor();
			else if(nearestneighbors==-1 || (nearestneighbors>t && !maxDecomposition))c=new Color(1f,0f,0f);
			else
			{
				c=shade;
			}
	...
	public class GrayNode extends Node
	{
		...
		{
			if(depth>maxDepth)throw new DepthExceededException(this);
			if(nw.contains(p))nw=nw.insert(p);
			else if(ne.contains(p))ne=ne.insert(p);
			else if(sw.contains(p))sw=sw.insert(p);
			else if(se.contains(p))se=se.insert(p);
		...
		public BlackNode getOnlyBlackNode()
		{
			...
			if(nw instanceof BlackNode){count++;toReturn=(BlackNode)nw;}
			if(ne instanceof BlackNode){count++;toReturn=(BlackNode)ne;}
			if(sw instanceof BlackNode){count++;toReturn=(BlackNode)sw;}
			if(se instanceof BlackNode){count++;toReturn=(BlackNode)se;}
			
			if(count==1)return toReturn;
			else return null;
		}
		...
		{
			Node newNode=null;
			if(nw instanceof WhiteNode && ne instanceof WhiteNode && sw instanceof WhiteNode && se instanceof WhiteNode)
			{
		...
		{
			if(nw==old)nw=newn;
			else if(ne==old)ne=newn;
			else if(sw==old)sw=newn;
			else if(se==old)se=newn;
		}
	}
	
	public class LightGrayNode extends GrayNode
	{
		public LightGrayNode(double m1,double m2,double m3,double m4,GrayNode parent) throws DepthExceededException
		{
			super(m1,m2,m3,m4,parent);
		}
	...
	public class WhiteNode extends Node
	{
		...
		public GrayNode split() throws DepthExceededException
		{
			...
			WhiteNode nw=new WhiteNode(minx,miny,(this.maxx+this.minx)/2,(this.maxy+this.miny)/2,toReturn);
			WhiteNode ne=new WhiteNode((this.maxx+this.minx)/2,miny,this.maxx,(this.maxy+this.miny)/2,toReturn);
			WhiteNode sw=new WhiteNode(minx,(this.maxy+this.miny)/2,(this.maxx+this.minx)/2,this.maxy,toReturn);
			WhiteNode se=new WhiteNode((this.maxx+this.minx)/2,(this.maxy+this.miny)/2,this.maxx,this.maxy,toReturn);
	...
	public class BlackNode extends Node
	{
		...
		DPoint pt;
		public BlackNode(double m1,double m2,double m3,double m4,GrayNode parent,DPoint p) throws DepthExceededException
		{
			super(m1,m2,m3,m4,parent);
			this.pt=p;
		}
