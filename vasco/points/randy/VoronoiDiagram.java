...
public class VoronoiDiagram extends PointStructure implements Indexable {
	...
	public ArrayList getClippedLines() {
		...
		while (it.hasNext()) {
			...
			for (DTriangle element : tri.adjacencies) {
				if (element != null) {
					DPoint p1 = tri.circumcircleCenter;
					DPoint p2 = element.circumcircleCenter;

					if (!pc.can.contains(p1)) {
						if (pc.can.height < p1.y)
							p1 = new DPoint(fixPoint(p1.x, p1.y, p2.x, p2.y, pc.can.height), pc.can.height);
						if (0 > p1.y)
							p1 = new DPoint(fixPoint(p1.x, p1.y, p2.x, p2.y, 0), 0);
						if (pc.can.width < p1.x)
							p1 = new DPoint(pc.can.width, fixPoint(p1.y, p1.x, p2.y, p2.x, pc.can.width));
						if (0 > p1.x)
							p1 = new DPoint(0, fixPoint(p1.y, p1.x, p2.y, p2.x, 0));
					}
					if (!pc.can.contains(p2)) {
						if (pc.can.height < p2.y)
							p2 = new DPoint(fixPoint(p2.x, p2.y, p1.x, p1.y, pc.can.height), pc.can.height);
						if (0 > p2.y)
							p2 = new DPoint(fixPoint(p2.x, p2.y, p1.x, p1.y, 0), 0);
						if (pc.can.width < p2.x)
							p2 = new DPoint(pc.can.width, fixPoint(p2.y, p2.x, p1.y, p1.x, pc.can.width));
						if (0 > p2.x)
							p2 = new DPoint(0, fixPoint(p2.y, p2.x, p1.y, p1.x, 0));
					}
	...
	public void drawContents(DrawingTarget dt, Rectangle r, Color c) {
		...
		if (showDelaunay)
			delaunay.drawContents(dt, r, Color.yellow);
		...
		for (Object element : delaunay.sites) {
			DPoint dp = (DPoint) element;
			dp.draw(dt);
		}
	...
	public Vector getAllLines() {
		...
		while (it.hasNext()) {
			...
			for (DTriangle element : tri.adjacencies) {
				if (element != null) {
					DLine l = new DLine(tri.circumcircleCenter, element.circumcircleCenter);
					toReturn.add(l);
				}
			}
		}
	...
	public class ShowPM2Combo implements ItemListener {
		public ShowPM2Combo(TopInterface ti) {
			...
			maxDChoice.addItem("Yes");
			if (showPM2)
				maxDChoice.setSelectedIndex(1); // No is default
			else
				maxDChoice.setSelectedIndex(0);
			maxD.add("East", maxDChoice);
			maxDChoice.addItemListener(this);
			ti.getPanel().add(maxD);
		}
