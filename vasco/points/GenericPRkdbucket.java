...
public abstract class GenericPRkdbucket extends PointStructure implements MaxDecompIface, BucketIface {
	...
	public boolean Insert(DPoint qu) {
		...
		if (ROOT == null) {
			...
			res = insert(qu, ROOT, true, wholeCanvas.x + wholeCanvas.width / 2, wholeCanvas.y + wholeCanvas.height / 2,
	...
	public void Delete(DPoint qu) {
		...
		if (p != null) {
			if (ROOT.isIn(p)) {
				...
				delete(p, ROOT, true, wholeCanvas.x + wholeCanvas.width / 2, wholeCanvas.y + wholeCanvas.height / 2,
	...
	public void drawContents(DrawingTarget g, Rectangle view) {
		if (ROOT != null)
			drawC(ROOT, g, true, wholeCanvas.x + wholeCanvas.width / 2, wholeCanvas.y + wholeCanvas.height / 2,
					wholeCanvas.width, wholeCanvas.height);
	}
	...
	private void search(PRkdbucketNode r, QueryObject query, boolean xcoord, DRectangle block, int mode,
			SearchVector res) {
	...
	abstract boolean insert(DPoint p, PRkdbucketNode r, boolean xcoord, double cx, double cy, double sx, double sy,
			int md);

	private void delete(DPoint del, PRkdbucketNode r, boolean xcoord, double cx, double cy, double sx, double sy) {
		if (xcoord) {
			if (del.x > cx) {
				if (r.right.isIn(del))
		...
		if (r.left.nodetype != GRAY && r.right.nodetype != GRAY
			...
			r.points = new Vector();
			r.nodetype = (r.left.points.size() + r.right.points.size() == maxBucketSize) ? BLACK : WHITE;
	...
	private void drawC(PRkdbucketNode r, DrawingTarget g, boolean xcoord, double cx, double cy, double sx, double sy) {
	...
	class PRkdbucketIncNearest {
		...
		// element.block.y,
		// element.block.width / 2, element.block.height);
		// q.Enqueue(new PRkdbucketQueueElement(qu.distance(dr), element.r.right, dr,
		// !element.xcoord));
		// }
		// } else { // ycoord
		// if (element.r.left != null) {
		...
		// q.Enqueue(new PRkdbucketQueueElement(qu.distance(dr), element.r.left, dr,
		// !element.xcoord));
		// }
		// if (element.r.right != null) {
		...
		// element.block.width, element.block.height / 2);
		// q.Enqueue(new PRkdbucketQueueElement(qu.distance(dr), element.r.right,
		// !element.xcoord));
		// }
		// }
		// }
		...
		DPoint[] Query(QueryObject qu, SearchVector ret, double dist, int nrelems) {
			...
			while (!q.isEmpty()) {
				...
				if (element.isElem) {
					...
					ret.addElement(new NNElement(new NNDrawable(element.pnt, counter++), element.key, q.makeVector()));
				} else {
					ret.addElement(new NNElement(new YellowBlock(element.block, false), element.key, q.makeVector()));
					...
					if (element.r == null) {
						...
						if (element.xcoord) {
							// if (element.r.left != null) {
							dr = new DRectangle(element.block.x, element.block.y, element.block.width / 2,
									element.block.height);
							q.Enqueue(new PRkdbucketQueueElement(qu.distance(dr), element.r.left, dr, !element.xcoord));
							// }
							// if (element.r.right != null) {
							dr = new DRectangle(element.block.x + element.block.width / 2, element.block.y,
									element.block.width / 2, element.block.height);
							q.Enqueue(
									new PRkdbucketQueueElement(qu.distance(dr), element.r.right, dr, !element.xcoord));
							// }
						} else { // ycoord
							// if (element.r.left != null) {
							dr = new DRectangle(element.block.x, element.block.y, element.block.width,
									element.block.height / 2);
							q.Enqueue(new PRkdbucketQueueElement(qu.distance(dr), element.r.left, dr, !element.xcoord));
							// }
							// if (element.r.right != null) {
							dr = new DRectangle(element.block.x, element.block.y + element.block.height / 2,
									element.block.width, element.block.height / 2);
							q.Enqueue(
									new PRkdbucketQueueElement(qu.distance(dr), element.r.right, dr, !element.xcoord));
							// }
