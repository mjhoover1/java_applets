...
public class PriorityTree extends PointStructure {
  ...
  DPoint GetPoint(Node2d r, Vector assigned) {
    ...
    for (int i = 0; i < subtree.size() - 1; i++)  
      for (int j = 0; j < subtree.size() - 1; j++)
	if (compareToY((DPoint)subtree.elementAt(j), (DPoint)subtree.elementAt(j+1)) < 0) {
	  DPoint p;
	  p = (DPoint)subtree.elementAt(j+1);
	  subtree.setElementAt(subtree.elementAt(j), j+1);
	  subtree.setElementAt(p, j);
	}
    */
    ...
    for (int i = 0; i < subtree.size(); i++) {
      if (!assigned.contains((DPoint)subtree.elementAt(i))) {
  ...
  void Y_SEARCH(Node2d T, QueryObject searchRect, double minx, double maxx, double height, 
		int mode, SearchVector v, Vector searchVector) {
    /* Perform a one-dimensional range search for the semi-infinite interval
       |[BY : infinity]| in the priority search tree rooted at |T|. */
  ...
  class PriorityIncNearest {
	...
	if (e.isElem) {
		...
		q.Enqueue(new PriorityQueueElement(qu.distance(b), e.r.LEFT, b));
	      ...
	      if (e.r.RIGHT != null) {
		b = new DRectangle(e.r.midrange, e.box.y, e.box.x + e.box.width - e.r.midrange, e.r.heapMax.y);
