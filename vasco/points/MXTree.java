...
public class MXTree extends PointStructure implements MaxDecompIface {
	...
	return new DPoint(wholeCanvas.x + wholeCanvas.width * p.x / (double)canvasWidth,
			  wholeCanvas.y + wholeCanvas.height * p.y / (double)canvasWidth);
    }
	...
	v.addElement(new SVElement(new YellowBlock(toDRectangle(new Rectangle((int)(cx - w/2.0), 
									      (int)(cy - w/2.0), 
									      w, 
									      w)), 
	...
	DRectangle sw = new DRectangle(toDRectangle(new Rectangle((int)(cx - w/2.0), 
								  (int)(cy - w/2.0), w/2, w/2)));
	...
	MXIncNearest(MXNode rt) {
	    q = new MXQueue();
	    q.Enqueue(new MXQueueElement(0, rt, 
					 wholeCanvas.width / 2, 
					 wholeCanvas.height / 2, 
					 wholeCanvas.width));
	}
	...
	DPoint[] Query(QueryObject dqu, SearchVector ret, double dist, int nrelems) {
		...
		if (element.isElem) {
			...
			break;
		    pts.addElement(epnt);
		    ret.addElement(new NNElement(new NNDrawable(new DPoint(element.cx, element.cy), 
								counter++), element.key,
						 q.makeVector()));
		} else {
		    ret.addElement(new NNElement(new YellowBlock(new DRectangle(element.cx - element.w/2, 
										element.cy - element.w/2, 
										element.w, 
										element.w), 
								 false), element.key,
						 q.makeVector()));
		    if (element.r.NODETYPE == BLACK) {
			q.Enqueue(new MXQueueElement(dqu.distance(new DPoint(element.cx - element.w / 2.0,
									     element.cy - element.w / 2.0)), 
						     element.r,
						     element.cx - element.w/2, 
						     element.cy - element.w/2)); 
		    } else if (element.r.NODETYPE == GRAY) {
			for (int i = 0; i < 4; i++) 
			    if (element.r.SON[i] != null)
				q.Enqueue(new MXQueueElement(dqu.distance(new DRectangle(element.cx + element.w / XF[i] - element.w/4.0, 
										     element.cy + element.w / YF[i] - element.w/4.0, 
										     element.w/2, element.w/2)), 
							     element.r.SON[i],
							     element.cx + element.w / XF[i], 
							     element.cy + element.w / YF[i], 
							     element.w/2));
		    }
		}
	    }
	...
	MXDPoint QueryReal(QueryObject dqu) {
		...
		MXQueueElement element = q.Dequeue();
		if (element.isElem) {
		    return new MXDPoint(element.r.point, 
					new DPoint(element.cx, element.cy));
		} else {
		    if (element.r.NODETYPE == BLACK) {
			q.Enqueue(new MXQueueElement(dqu.distance(element.r.point), 
						     element.r,
						     element.cx - element.w/2, 
						     element.cy - element.w/2));
