...
public class PTTree extends PointStructure {
	...
	private void search(PTNode R, QueryObject qu, DRectangle block, int mode, SearchVector v) {
		v.addElement(new SVElement(
				new YellowBlock(block,
						R == null || (R.SON[0] == null && R.SON[1] == null && R.SON[2] == null && R.SON[3] == null)),
				searchVector));
		...
		DRectangle ne = new DRectangle(R.pnt.x, R.pnt.y, block.x + block.width - R.pnt.x,
				block.y + block.height - R.pnt.y);
	...
	private DPoint FindCandidate(PTNode P, int Q) {
		if (P == null) {
			switch (Q) {
			case SW:
				return new DPoint(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY);
			case NW:
				return new DPoint(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
			case SE:
				return new DPoint(Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY);
			case NE:
				return new DPoint(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
			}
			return null;
		} else {
		...
		DPoint[] Query(QueryObject qu, SearchVector ret, double dist, int nrelems) {
			...
			while (!q.isEmpty()) {
				...
				if (e.isElem) {
					...
					if (e.r.SON[1] != null) {
						b = new DRectangle(e.r.pnt.x, e.r.pnt.y, e.box.x + e.box.width - e.r.pnt.x,
								e.box.y + e.box.height - e.r.pnt.y);
