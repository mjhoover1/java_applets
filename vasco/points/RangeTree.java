...
public class RangeTree extends PointStructure {
	...
	void Create1dTree(Node2d r) {
		...
		/*
		 * for (int i = 0; i < arr.size() - 1; i++) for (int j = 0; j < arr.size() - 1;
		 * j++) if (compareToY((DPoint)arr.elementAt(j), (DPoint)arr.elementAt(j+1)) >
		 * 0) { DPoint p; p = (DPoint)arr.elementAt(j+1);
		 * arr.setElementAt(arr.elementAt(j), j+1); arr.setElementAt(p, j); }
		 *
		 */
		Node1d oldar[];
		Node1d newar[];
	...
	public SearchVector Search(QueryObject searchable, int mode) {
		...
		while (T.point == null) {
			v.addElement(new SVElement(
					new YellowBlock(minleft, wholeCanvas.y, maxright, wholeCanvas.y + wholeCanvas.height, false),
					searchVector));
		...
		if (T.point != null) {
			...
			T = T.LEFT;
			while (T.point == null) {
				searchVector.addElement(new DRectangle(minright, wholeCanvas.y, maxright - minright,
						wholeCanvas.y + wholeCanvas.height));
				searchVector.addElement(
						new DRectangle(minleft, wholeCanvas.y, maxleft - minleft, wholeCanvas.y + wholeCanvas.height));
				v.addElement(new SVElement(new YellowBlock(minleft, wholeCanvas.y, maxleft - minleft,
						wholeCanvas.y + wholeCanvas.height, false), searchVector));
				searchVector.removeElementAt(searchVector.size() - 1);

				if (LX <= T.midrange) {
					searchVector.addElement(new DRectangle(minleft, wholeCanvas.y, T.midrange - minleft,
							wholeCanvas.y + wholeCanvas.height));
					searchVector.addElement(new DRectangle(T.midrange, wholeCanvas.y, maxleft - T.midrange,
							wholeCanvas.y + wholeCanvas.height));
					v.addElement(new SVElement(new YellowBlock(T.midrange, wholeCanvas.y, maxleft - T.midrange,
							wholeCanvas.y + wholeCanvas.height, true), searchVector));
					searchVector.removeElementAt(searchVector.size() - 1);

					searchVector.addElement(new DRectangle(minright, wholeCanvas.y, maxright - minright,
							wholeCanvas.y + wholeCanvas.height));

					v.addElement(new SVElement(new GreenPoints(greenPoints(T.RIGHT.range_tree, LY, RY)), searchVector));
			...
			T = Q.RIGHT;
			while (T.point == null) {
				v.addElement(new SVElement(new YellowBlock(minright, wholeCanvas.y, maxright - minright,
						wholeCanvas.y + wholeCanvas.height, false), searchVector));
				if (T.midrange <= RX) {
					searchVector.addElement(new DRectangle(T.midrange, wholeCanvas.y, maxright - T.midrange,
							wholeCanvas.y + wholeCanvas.height));
					v.addElement(new SVElement(new YellowBlock(minright, wholeCanvas.y, T.midrange - minright,
							wholeCanvas.y + wholeCanvas.height, true), searchVector));

					v.addElement(new SVElement(
							new GreenPoints(
									greenPoints(T.LEFT.range_tree, searchRect.y, searchRect.y + searchRect.height)),
							searchVector));
