...
public class InsertBlockCell implements HistoryElmInterface {
	...
	public Vector switchGrid(int oldRes, int oldCellSize, int[][] oldGrid, Grid newGrid) {
		...
		if (oldRes < newGrid.res) {
			...
			v.addElement(new InsertBlockCell(
					new Rectangle(block.x * diff, block.y * diff, diff * block.width, diff * block.height), 1));
			/* for deleted ones */
			for (x = block.x; x < block.x + block.width; x++)
				for (y = block.y; y < block.y + block.height; y++)
					if (oldGrid[y][x] == 0)
						v.addElement(new InsertBlockCell(new Rectangle(x * diff, y * diff, diff, diff), 0));
		} // if
		else {
			diff = (int) (Math.pow(2, oldRes - newGrid.res));
			Point p = newGrid.getGridCoor(new Point(block.x * oldCellSize, block.y * oldCellSize));
			int w = block.width / diff;
			if (w == 0)
				w = 1;
			int h = block.height / diff;
			if (h == 0)
				h = 1;
			v.addElement(new InsertBlockCell(new Rectangle(p.x, p.y, w, h), 1));
			...
			for (x = 0; x < w; x++) {
				for (y = 0; y < h; y++) {
					for (xx = 0; xx < diff; xx++) {
						for (yy = 0; yy < diff; yy++) {
							if (oldGrid[block.y + y * diff + yy][block.x + x * diff + xx] == 1)
								break;
						} // for yy
						if (yy != diff)
							break;
					...
					if (xx == diff) {
						p = newGrid.getGridCoor(
								new Point((block.x + x * diff) * oldCellSize, (block.y + y * diff) * oldCellSize));
						v.addElement(new DeleteBlockCell(new Rectangle(p.x, p.y, 1, 1)));
					} // if
