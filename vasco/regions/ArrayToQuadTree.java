...
public class ArrayToQuadTree {
	...
	protected Node construct(int level, int x, int y, MyRefInt c, ConvertVector sv, Node dRoot) {
		int d, i;
		Node[] p = new Node[4];
		...
		if (level == 0) { /* at a pixel */
			...
			if (sv.inActiveRegion(scr.x, scr.y)) {
				...
				sv.addElement(new ConvertElement(
						new ColorGrids(grid, new Point(x - 1, y - 1), c.getValue(), dRoot.getCopy()), new Vector()));
			} else {
				Node.insert(dRoot, scr, array.getColor(x - 1, y - 1), grid.cellSize);
				sv.addElement(new ConvertElement(new ColorGrids(grid, new Point(x - 1, y - 1), c.getValue(), dRoot),
						new Vector()));
			}
			...
			if ((v != Node.GRAY) && (v == t[Node.NE].getValue()) && (v == t[Node.SW].getValue())
					&& (v == t[Node.SE].getValue())) {
				c.setValue(v);
				return null;
			} else {
