...
abstract public class RegionStructure extends SpatialStructure 
  ...
  RegionStructure(GenericCanvas rCanvas, DRectangle can, DrawingTarget dt, 
		  TopInterface ti, RebuildTree r, Grid g, int m) {
    super(can, ti, r);     
  ...
  public boolean insert(Node node, Point p, int color){
	...
	node.split(); 
	return (insert(node.child[0], p, color) || 
		insert(node.child[1], p, color) ||
		insert(node.child[2], p, color) || 
		insert(node.child[3], p, color));
      }   
      else{
	if (color == 1)
	  node.color = 1;
  ...
  public Node quadSearch(Node node, Point p, int size, Vector nearest, double dist){
    ...
    Rectangle r = new Rectangle(node.x + (node.size / 2) - (size / 2),
				node.y + (node.size / 2) - (size / 2),
				size, size);
 
    /* calculate the distance from the center to the Point p */
    d = Math.pow(node.x + (node.size / 2) - p.x, 2) +
      Math.pow(node.y + (node.size / 2) - p.y, 2);
    if (dist == -1){
  ...
  public CursorStyle mouseMoved(int rx, int ry, int x, int y, int op, SelectedRect sRect){
    ...
    if(op != 12 && mode == QUAD_MODE){ 
      ...
      cs.add(new ValidGridCursor(
				 new Rectangle(nNode.x + (nNode.size / 2) - (size / 2),
					       nNode.y + (nNode.size / 2) - (size / 2),
					       size, size),
				 Colors.NEAREST_NODE));
    }
	...
	selected = new Rectangle(sp.x, sp.y, ep.x - sp.x + 1, ep.y - sp.y + 1);
      ...
      cs.add(new ValidGridCursor(new Rectangle(node.x + 1, node.y + 1, 
