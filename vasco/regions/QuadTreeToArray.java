		...
		while (!s.isEmpty()) {
			...
			if (node.isLeaf) {
				...
				container.addElement(new PermenantElement(sv,
						new GridElement(grid, new Rectangle(sP.x, sP.y, eP.x - sP.x + 1, eP.y - sP.y + 1))));

				container.addElement(new RectangleElement(new Rectangle(node.x, node.y, node.size, node.size),
