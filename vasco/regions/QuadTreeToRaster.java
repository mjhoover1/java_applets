...
public class QuadTreeToRaster {
	...
	protected void outRow(Node p, int row, int ll, ConvertVector sv) {
		...
		container.addElement(
				new RectangleElement(new Rectangle(p.x, p.y, p.size, p.size), Color.green, 1, false, false));

		container.addElement(new RectangleElement(
				new Rectangle(p.x + 1, row * grid.getCellSize() + 1, p.size - 2, grid.getCellSize() - 2), Color.blue, 1,
				false, false));
		...
		do {
			...
			if ((q.node != null) && (q.node.isGray())) {
				...
				find2DBlock(q, 0, width, row, row + width - (row % width), newWidth);
			...
			if (q.node != null) {
				...
				container.addElement(new RectangleElement(new Rectangle(q.node.x + 1, row * grid.getCellSize() + 1,
						q.node.size - 2, grid.getCellSize() - 2), Color.blue, 1, false, false));
	...
	protected void find2DBlock(RefNode p, int x, int xfar, int y, int yfar, RefInt w) {
		int[] XF = { 1, 0, 1, 0 };
		int[] YF = { 1, 1, 0, 0 };
		int q;
	...
	public void qtGteqEdgeNeighbor2(Node p, int i, RefNode q, RefInt l) {
		int tmp;
