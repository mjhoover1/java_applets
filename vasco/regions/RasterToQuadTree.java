...
public class RasterToQuadTree {
	...
	protected void oddRow(int row, int w, int[] q, Node r, ConvertVector sv) {
		...
		if (sv.inActiveRegion(scr.x, scr.y)) {
			...
			sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[0], root.getCopy()), new Vector()));
		} else {
			root.completeNode(0, 0, 512, 0);
			sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[0], root), new Vector()));
		}
		...
		for (int i = 1; i < w; i++) {
			...
			if (sv.inActiveRegion(scr.x, scr.y)) {
				root.completeNode(0, 0, 512, 0);
				sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[i], root.getCopy()), new Vector()));
			} else {
				root.completeNode(0, 0, 512, 0);
				sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[i], root), new Vector()));
			}

		}
	}

	protected Node evenRow(int i, int w, boolean lastRow, int[] q, RefNode first, ConvertVector sv) {
		int j;
		...
		for (j = 0; j < w - 1; j++) {
			...
			if (sv.inActiveRegion(scr.x, scr.y)) {
				root.completeNode(0, 0, 512, 0);
				sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[j], root.getCopy()), new Vector()));
			} else {
				root.completeNode(0, 0, 512, 0);
				sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[j], root), new Vector()));
			}
		...
		if (sv.inActiveRegion(scr.x, scr.y)) {
			root.completeNode(0, 0, 512, 0);
			sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[w - 1], root.getCopy()), new Vector()));
		} else {
			root.completeNode(0, 0, 512, 0);
			sv.addElement(new ConvertElement(new ColorGrids(grid, g, q[w - 1], root), new Vector()));
		}
	...
	protected void merge(int i, int j, Node p) {
		...
		t = p.child[0].color;
		while (((i % 2) == 0) && ((j % 2) == 0) && (p.child[0].isLeaf) && (t == p.child[1].color) && (p.child[1].isLeaf)
				&& (t == p.child[2].color) && (p.child[2].isLeaf) && (t == p.child[3].color) && (p.child[3].isLeaf)) {
			i = i / 2;
			j = j / 2;
	...
	protected Node addEdgeNeighbor(Node q, int e, int c, ConvertVector sv) {
		...
		if (q.parent == null) { /* nearest common ancestor does not exists */
			...
			q.sonOf = typeQ;
			/* create three sons */
			p.child[Node.opquad(q.sonOf)] = Node.create(p, Node.opquad(typeQ), c);
			p.child[Node.opquad(Node.reflect(e, typeQ))] = Node.create(p, Node.opquad(Node.reflect(e, typeQ)), c);
			p.child[Node.reflect(e, typeQ)] = (Node.create(p, Node.reflect(e, typeQ), c));
	...
	public Node convert(ConvertVector sv) {
		...
		first.node = evenRow(i, width, (raster.getRow(2) == null), raster.getRow(1), tmpNode, sv);

		while ((rowP = raster.getRow(i + 1)) != null) {
			oddRow(i + 1, width, rowP, first.getValue(), sv);
			i = i + 2;
			tmpNode.node = addEdgeNeighbor(first.getValue(), Node.S, -1, sv);
			first.setValue(evenRow(i, width, (raster.getRow(i + 1) == null), raster.getRow(i), tmpNode, sv));
		}
