...
public class QuadTreeToChain {
	...
	public static void convert(Node node, Grid g, Vector code, ConvertVector sv, StructureBox si) {
		...
		while (p.node.isGray()) {
			if (sv != null) {
				...
				container.addElement(new RectangleElement(new Rectangle(p.node.x, p.node.y, p.node.size, p.node.size),
	...
	public static boolean marked(Vector markList, Node p, Node q) {
		...
		for (int x = 0; x < markList.size(); x++) {
			...
			if (((elm.p == p) && (elm.q == q)) || ((elm.p == q) && (elm.q == p)))
				return true;
		}
		return false;
	}
	...
	public static void nextLink(Node node, Grid g, Vector code, ConvertVector sv, StructureBox si, Vector markList,
			RefNode p, RefInt lp, RefNode q, RefInt lq, int e) {
		int min, i;
		RefInt lx, ly;
		RefNode x, y;
		ContainerElement container = null;
		...
		if (sv != null) {
			...
			le = new LineElement(g, new Point(intersect.x, intersect.y),
					new Point(intersect.x + intersect.width, intersect.y + intersect.height), ChainCode.link(e),
					Color.blue, null, (int) Math.pow(2, min), true);

			container.addElement(le);
			container.addElement(new RectangleElement(new Rectangle(p.node.x, p.node.y, p.node.size, p.node.size),
					Color.green, 1, false, false));
			container.addElement(new RectangleElement(new Rectangle(q.node.x, q.node.y, q.node.size, q.node.size),
					Color.green, 1, false, false));
		...
		if (aligned(p.node, lp.value, q.node, lq.value, Node.cedge(e))) {
			lx.value = lp.value;
			qtVertexEdgeNeighbor2(p, Node.cedge(e), Node.quad(Node.cedge(e), e), x, lx);
			if (x.node.isWhite()) {
				nextLink(node, g, code, sv, si, markList, p, lp, x, lx, Node.cedge(e));
			} /* WHITE */
			else { /* !WHITE */
				ly.value = lq.value;
				qtVertexEdgeNeighbor2(q, Node.cedge(e), Node.quad(Node.cedge(e), Node.opedge(e)), y, ly);
			...
			lx.value = lq.value;
			qtVertexEdgeNeighbor2(q, Node.cedge(e), Node.quad(Node.cedge(e), Node.opedge(e)), x, lx);
			if (x.node.isWhite())
	...
	public static void qtVertexEdgeNeighbor2(RefNode p, int e, int v, RefNode q, RefInt l) {
	...
	public static void qtGteqEdgeNeighbor2(Node p, int i, RefNode q, RefInt l) {
		int tmp;
