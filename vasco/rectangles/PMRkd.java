...
class PMRkd extends RectangleStructure implements BucketIface, MaxDecompIface {
  ...
  private void search(RNode r, QueryObject query, boolean xcoord, DRectangle block, SearchVector v, int mode) {
	...
	DRectangle right = new DRectangle(block.x + block.width /2, block.y, 
	...
	DRectangle right = new DRectangle(block.x, block.y + block.height / 2, 
  ...
  boolean insert (DRectangle p, RNode r, boolean xcoord, DRectangle block, int md) {
	...
	if (p.x < block.x + block.width / 2)  // x coordinate
	  ok = insert(p, r.son[0], !xcoord, 
		      new DRectangle(block.x, block.y, block.width / 2, block.height), md - 1) && ok;
	if (p.x + p.width > block.x + block.width / 2)
	  ok = insert(p, r.son[1], !xcoord, 
		      new DRectangle(block.x + block.width/2, block.y, 
				    block.width/2, block.height), 
		      md - 1) && ok;
	...
	if (p.y < block.y + block.height / 2)
	  ok = insert(p, r.son[0], !xcoord, 
		      new DRectangle(block.x, block.y, block.width, block.height/2), 
		      md - 1) && ok;
      }
      return ok;
    }
  ...
  private void delete(DRectangle del, RNode r, boolean xcoord, DRectangle block) {
    ...
    if (r.son[0].NODETYPE != GRAY && r.son[1].NODETYPE != GRAY &&
	 r.son[1].r.size() + r.son[1].r.size() <= maxBucketSize) {
      r.r = new Vector();
      r.NODETYPE = (r.son[0].r.size() + r.son[1].r.size() == maxBucketSize) ? 
	BLACK : WHITE;
  ...
  private void drawC(RNode r, DrawingTarget g, boolean xcoord, DRectangle block, Rectangle view) {
    ...
    */
	...
	g.drawLine(block.x, block.y + block.height/2, block.x + block.width, block.y + block.height/2);
      ...
      if (xcoord) {
	drawC(r.son[0], g, !xcoord, 
	      new DRectangle(block.x, block.y, block.width/2, block.height), view);
	drawC(r.son[1], g, !xcoord, 
	      new DRectangle(block.x + block.width/2, block.y, block.width/2, block.height), view);
      } else {
	drawC(r.son[0], g, !xcoord, 
	      new DRectangle(block.x, block.y, block.width, block.height/2), view);
	drawC(r.son[1], g, !xcoord, 
  ...
  class PMRkdQueue {
	...
	PMRkdQueueElement q2 = (PMRkdQueueElement)v.elementAt(i);
	  ...
	  /*
	if (q1.key == q2.key) {
	    if (q1 instanceof PMRkdQLeaf && q2 instanceof PMRkdQINode) {
		v.setElementAt(q2, i - 1);
		v.setElementAt(q1, i);
	    } else if (q1 instanceof PMRkdQLeaf && q2 instanceof PMRkdQLeaf) {
		PMRkdQLeaf ql1 = (PMRkdQLeaf)q1;
		PMRkdQLeaf ql2 = (PMRkdQLeaf)q2;
		if (ql1.rect.x > ql2.rect.x ||
		    (ql1.rect.x == ql2.rect.x && ql1.rect.y > ql2.rect.y) ||
	...
	while(!q.isEmpty()) {
	  ...
	  if (element instanceof PMRkdQLeaf) {
	      ...
	      ret.addElement(new NNElement(new NNDrawable(lf.rect, counter++), 
					   lf.keys[0], q.makeVector()));
	  } else {
	    PMRkdQINode in = (PMRkdQINode)element;
	    ret.addElement(new NNElement(new YellowBlock(in.block, false), 
					 in.keys[0], q.makeVector()));
		...
		if (in.r.son[i] != null) {
		  DRectangle dr = new DRectangle(in.block.x + (in.xcoord ? cf[i] * in.block.width : 0),
						 ...
						 in.block.width / (in.xcoord ? 2 : 1),
						 in.block.height / (!in.xcoord ? 2 : 1));
